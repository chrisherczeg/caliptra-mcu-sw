...
impl McuHwModel for ModelEmulated {
        ...
        Self: Sized,
    {
        ...
        let (network_cpu, network_uart_output) = if !params.network_rom.is_empty() {
            ...
            let network_args = caliptra_mcu_emulator_periph::NetworkRootBusArgs {
            ...
            let mut network_cpu = Cpu::new(
                ...
                network_clock,
                network_pic,
                caliptra_mcu_emulator_consts::NETWORK_CPU_ARGS,
                Rc::new(Cell::new(0)),
            );
...
impl Drop for ModelEmulated {
    fn drop(&mut self) {
        ...
        {
            let cov_path =
                std::env::var(caliptra_mcu_coverage::MCU_COVERAGE_PATH).unwrap_or_default();
            if !cov_path.is_empty() {
                let bitmaps = self.cpu.code_coverage.code_coverage_bitmap();
                let _ = caliptra_mcu_coverage::dump_coverage_to_file(
                    &cov_path,
                    self._rom_image_tag,
                    bitmaps.rom,
                );
